...
public class OrderService {
    ...
    public void newOrderPush(String shopId, @Valid NewOrderBean newOrderBean) {
        ...
        List<NewOrderDetailBean> newOrderDetaiList = newOrderBean.getDetail();
        ...
        pushOrder.setOnlinePaid(PayTypeEnum.onlinePay(newOrderBean.getPayType())
                ? StatusEnum.ENABLE.getStatus().toString() : StatusEnum.DISENABLE.getStatus().toString());
        ...
        pushOrder.setStatus(autoConfirm ? OrderStatusEnum.mt_business_confirmed.getCode()
                : OrderStatusEnum.get(newOrderBean.getStatus().toString()).getText());
        ...
        try {
            ...
            pushResult = takeawayClient.pushNewOrder(pushOrderJson);
        } catch (Exception e) {
        ...
        order.setStatus(OrderStatusEnum.get(newOrderBean.getStatus().toString()));
    ...
    public void refundOrderConfirm(String shopId, Long orderId, String reason) {
        ...
        if (OK.equals(result)) {
            ...
            try {
                ...
                logger.error("修改订单状态失败!orderId={}", orderId, e);
                throw new RuntimeException("美团确认退款成功,修改本地订单状态失败!" + e.getMessage());
            }
            // TODO 调用jpa接口修改退款单状态
        } else {
    ...
    public void refundOrderPush(String shopId, @Valid RefundOrderBean bean) {
        ...
        try {
            ...
            pushResult = takeawayClient.pushRefundOrder(JsonFormatUtil.toJSONString(pushRefundOrder));
        } catch (Exception e) {
